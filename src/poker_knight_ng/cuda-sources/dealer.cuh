...
constexpr std::uint32_t NO_FAILURE_DRAW_SLOT = UINT32_MAX;
struct DealerResult {
...
POKER_KNIGHT_NG_HD inline DealerStatus deal_adr0003(const std::uint8_t* known, std::uint32_t known_count,
  ...
  result->status=DealerStatus::INVALID_INPUT;
  result->failure_draw_slot=NO_FAILURE_DRAW_SLOT;
  if (!known || !(known_count==2 || known_count==5 || known_count==6 || known_count==7) || opponents<1 || opponents>6) { return result->status; }
  ...
  for(std::uint32_t slot=0;slot<draws;++slot) {
    ...
    for(;;) { std::uint32_t counter[4]={static_cast<std::uint32_t>(simulation_id),static_cast<std::uint32_t>(simulation_id>>32),slot,attempt}; std::uint32_t key[2]={k0,k1}, out[4], index=0; bool accepted=false; philox4x32_10(counter,key,out);
      ...
      if (step != DealerStatus::OK) {
        result->status=step;
        if (step == DealerStatus::RNG_REJECTION_EXHAUSTED) result->failure_draw_slot=slot;
