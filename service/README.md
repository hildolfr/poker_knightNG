...
Listener construction, stream adapter, socket ownership, and lifecycle cleanup are implemented in Phase 7B-L1; the Phase 7F deployment scaffold uses the runtime's direct bind, while socket activation is forbidden by ADR 0007. Structured logging and runtime process entrypoint scaffolding are now in Phase 7F. The execution boundary still adds no service execution timeout and never cancels admitted engine work; those are later checkpoints under ADR 0005.
